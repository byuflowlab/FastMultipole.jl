#!/bin/bash
# fm041a_submit_gpu_widen.sh — task 041a H200 job: pre-registered widened
# depth/K sweep + n densification + P sweep + leaf histograms
# (fm041a_gpu_widen.jl). Copy OUTSIDE the repo snapshot before sbatch:
#   cp to ~/fm041a_gpu_widen.sh, then `sbatch ~/fm041a_gpu_widen.sh`.
...
#SBATCH --mem=192G
#SBATCH -J fm041aG
#SBATCH -o /home/rander39/fm041aG_%j.out
...
echo "=== measurement of record: fm041a_gpu_widen.jl ==="
julia --project="$ENVDIR" -t 1 MATRIX_OPERATOR_REFACTOR/scripts/fm041a_gpu_widen.jl
