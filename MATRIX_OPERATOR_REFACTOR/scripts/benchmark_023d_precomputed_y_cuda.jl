# Task 023d: CUDA device-resident precomputed-y M2L benchmark.
#
# Compares four resident device variants on the recurring RadixFMMCache
# (device=true) lifecycle: the production concat path, the 023b factored
# whole-pass path, the 023d precomputed-y whole-pass path, and the 023d
# per-class reference baseline. Records construction cost, persistent/peak
# device memory, per-stage GPU timings, per-step wall time, device allocation
# on the M2L stage, angle/offset-class occupancy, and accuracy vs direct!.
# One CSV row per (variant, P, LH, N). Pattern: benchmark_023b_factored_cuda.jl.
#
# Env knobs:
#   FM023D_N      comma list of body counts        (default "150,2000,20000")
#   FM023D_P      comma list of expansion orders   (default "4,8,12")
#   FM023D_ELL    radix depth                      (default "3")
#   FM023D_REPS   timing repetitions (median)      (default "5")
#   FM023D_OUT    output CSV path
#   FM023D_DIRECT_MAX  max N for the direct! accuracy reference (default 20000)
#   FM023D_CHUNK  override both whole-pass chunk widths
#   FM023D_F32    also run a Float32 precomputed-y row per config ("1" to enable)
...
const NS = parse.(Int, split(get(ENV, "FM023D_N", "150,2000,20000"), ','))
const PS = parse.(Int, split(get(ENV, "FM023D_P", "4,8,12"), ','))
const ELL = parse(Int, get(ENV, "FM023D_ELL", "3"))
const REPS = parse(Int, get(ENV, "FM023D_REPS", "5"))
const DIRECT_MAX = parse(Int, get(ENV, "FM023D_DIRECT_MAX", "20000"))
const RUN_F32 = get(ENV, "FM023D_F32", "1") == "1"
const OUT = get(ENV, "FM023D_OUT", joinpath(@__DIR__, "..", "data",
    "precomputed_y_resident_m2l_cuda",
...
if haskey(ENV, "FM023D_CHUNK")
    chunk = parse(Int, ENV["FM023D_CHUNK"])
    FastMultipole.FACTORED_CUDA_CHUNK[] = chunk
    FastMultipole.PRECOMPUTED_CUDA_CHUNK[] = chunk
end

# (operator, m2l_strategy, whole-pass ref, whole-pass value) per variant
const VARIANTS = (
    ...
    (label="precomputed_y_perclass", operator=FactoredRotationM2L(),
        strategy=FastMultipole.PrecomputedFactoredYM2L(),
        wp_ref=FastMultipole.PRECOMPUTED_CUDA_WHOLE_PASS, wp=false),
)
...
function _measure_variant(P, LH, N, variant, ::Type{TF}) where TF
    ...
    elseif plan isa FastMultipole.ResidentM2LPrecomputedYPlan
        ...
        n_angles = length(plan.angle_thetas)
        nonempty_angles = count(>(0), plan.angle_counts)
    end
    return (host=gethostname(), gpu=CUDA.name(CUDA.device()),
        variant=variant.label, precision=string(TF), P=P, lh=LH, n=N, ell=ELL,
        ...
        n_angle_classes=n_angles, nonempty_angle_classes=nonempty_angles,
...
for N in NS, P in PS, LH in (false, true)
    for variant in VARIANTS
        ...
        push!(rows, row)
        @printf("%-22s %-8s P=%-2d LH=%-5s N=%-6d  m2l %8.3f ms  step %8.3f ms  mem %7.1f MB\n",
            row.variant, row.precision, row.P, string(row.lh), row.n, row.m2l_ms,
            row.full_step_ms, row.persistent_device_bytes / 1e6)
        flush(stdout)
    end
    if RUN_F32
        row = measure_variant(P, LH, N, VARIANTS[3]; precision=Float32)
        push!(rows, row)
        @printf("%-22s %-8s P=%-2d LH=%-5s N=%-6d  m2l %8.3f ms  step %8.3f ms  mem %7.1f MB\n",
            row.variant, row.precision, row.P, string(row.lh), row.n, row.m2l_ms,
            row.full_step_ms, row.persistent_device_bytes / 1e6)
