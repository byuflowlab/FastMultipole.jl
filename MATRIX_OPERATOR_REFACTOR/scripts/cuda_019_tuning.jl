# Task 019 GPU tuning benchmark: validates the dense fixed-stage GEMM concat M2L
# (whole-slab stacked-y + dense z GEMMs, fused gather/scatter kernels, preallocated
# chunk scratch) and measures per-stage timings and the chunk-size sweep on device.
# Protocol follows cuda_022_validation.jl; stage timings here are min-of-reps
# (worklist item 4), and results are also emitted as CSV under a CUDA_019_CSV marker.
using FastMultipole
...
CUDA019ScalarSystem(positions::Matrix{TF}, radii::Vector{TF}, strengths::Vector{TF}) where TF =
    CUDA019ScalarSystem(positions, radii, strengths, zeros(TF, length(strengths)), zeros(TF, 3, length(strengths)))

struct CUDA019DeviceSystem{TF,A,B,C}
    host::CUDA019ScalarSystem{TF}
    positions::A
    radii::B
    strengths::C
end

Base.eltype(::CUDA019ScalarSystem{TF}) where TF = TF
FastMultipole.get_n_bodies(system::CUDA019ScalarSystem) = size(system.positions, 2)
FastMultipole.data_per_body(::CUDA019ScalarSystem) = 5
FastMultipole.strength_dims(::CUDA019ScalarSystem) = 1
FastMultipole.get_position(system::CUDA019ScalarSystem{TF}, i) where TF =
    SVector{3,TF}(system.positions[1, i], system.positions[2, i], system.positions[3, i])

function FastMultipole.source_system_to_buffer!(buffer, i_buffer, system::CUDA019ScalarSystem, i_body)
...
function FastMultipole.buffer_to_target_system!(target_system::CUDA019ScalarSystem,
...
Base.eltype(::CUDA019DeviceSystem{TF}) where TF = TF
FastMultipole.get_n_bodies(system::CUDA019DeviceSystem) = FastMultipole.get_n_bodies(system.host)
FastMultipole.data_per_body(system::CUDA019DeviceSystem) = FastMultipole.data_per_body(system.host)
FastMultipole.strength_dims(system::CUDA019DeviceSystem) = FastMultipole.strength_dims(system.host)
FastMultipole.get_position(system::CUDA019DeviceSystem, i) = FastMultipole.get_position(system.host, i)
FastMultipole.residency(::CUDA019DeviceSystem) = DeviceResident()

function FastMultipole.source_to_buffer!(device_buffer, system::CUDA019DeviceSystem, sort_index)
...
function FastMultipole.buffer_to_target!(target_system::CUDA019DeviceSystem,
...
function make_system(::Type{TF}, n; seed=22022) where TF
    ...
    return CUDA019ScalarSystem(positions, radii, strengths)
end

function device_system(system::CUDA019ScalarSystem)
    return CUDA019DeviceSystem(
        system,
...
# Min-of-reps per-stage timings (019 worklist item 4): each stage is warmed once,
# then timed as the minimum over `reps` launches. Repeated stage launches only
# affect expansion values, not timing; callers must re-run one clean lifecycle
# before any parity comparison on `state`.
function _cuda_stage_times(state; reps=3)
    stage_fns = (
        t_b2m = FastMultipole._launch_cuda_b2m!,
        t_m2m = FastMultipole._launch_cuda_resident_m2m!,
        t_m2l = FastMultipole._launch_cuda_resident_m2l!,
        t_l2l = FastMultipole._launch_cuda_resident_l2l!,
        t_l2b = FastMultipole._launch_cuda_resident_l2b!,
    )
...
function throughput_sweep()
    rows = NamedTuple[]
    configs = (
        (; n=128, ell=3, P=2, policy=:parent, strategies=(:shared, :concat), chunks=(1 << 17,), reps=3, host_ref=true),
        (; n=10_000, ell=4, P=4, policy=:constp, strategies=(:concat,), chunks=(1 << 17,), reps=3, host_ref=true),
        (; n=100_000, ell=4, P=4, policy=:constp, strategies=(:concat,), chunks=(1 << 17, 1 << 18, 1 << 19), reps=3, host_ref=false),
        (; n=100_000, ell=4, P=8, policy=:constp, strategies=(:concat,), chunks=(1 << 17, 1 << 18, 1 << 19), reps=3, host_ref=false),
    )
    for cfg in configs
        ...
        for strat_name in cfg.strategies, chunk in (strat_name === :concat ? cfg.chunks : (0,))
            strat = strat_name === :shared ? SharedRotationM2L() : ConcatenatedFixedZM2L(chunk)
            ...
            stages = _cuda_stage_times(state; reps=cfg.reps)
            if cfg.host_ref
            ...
            push!(rows, (;
                cfg.n, cfg.ell, cfg.P, policy=cfg.policy, strategy=strat_name, chunk,
                routes, direct_pairs=length(list.direct_pairs),
                list_time, build_time, exec_time, exec_max, host_time, stages...,
            ))
        end
        # Release the big per-config device buffers before the next config.
        CUDA.reclaim()
    end
    return rows
end
...
if !FastMultipole.load_cuda_radix_lifecycle!()
    println("CUDA_019_CASES")
    println("CUDA_019_THROUGHPUT")
    println("CUDA_019_CSV")
    println("CUDA_019_RESULT")
    println("FAIL")
...
for row in sweep_rows
    @printf("n=%d ell=%d P=%d policy=%s strategy=%s chunk=%d routes=%d direct=%d list_t=%.3e build_t=%.3e exec_t=%.3e exec_max=%.3e host_t=%.3e b2m=%.3e m2m=%.3e m2l=%.3e l2l=%.3e l2b=%.3e\n",
        row.n, row.ell, row.P, row.policy, row.strategy, row.chunk, row.routes, row.direct_pairs,
        row.list_time, row.build_time, row.exec_time, row.exec_max, row.host_time,
        row.t_b2m, row.t_m2m, row.t_m2l, row.t_l2l, row.t_l2b)
end
println("CUDA_019_CSV")
println("n,ell,P,policy,strategy,chunk,routes,direct_pairs,list_time,build_time,exec_time,exec_max,host_time,t_b2m,t_m2m,t_m2l,t_l2l,t_l2b")
for row in sweep_rows
    @printf("%d,%d,%d,%s,%s,%d,%d,%d,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e\n",
        row.n, row.ell, row.P, row.policy, row.strategy, row.chunk, row.routes, row.direct_pairs,
        row.list_time, row.build_time, row.exec_time, row.exec_max, row.host_time,
        row.t_b2m, row.t_m2m, row.t_m2l, row.t_l2l, row.t_l2b)
end
