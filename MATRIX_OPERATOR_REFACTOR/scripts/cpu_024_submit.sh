#!/bin/bash
# Synchronize the exact task-024 snapshot and submit the CPU campaign.
# Run only after the user has explicitly approved cpu_024_run.sh.
set -euo pipefail
REMOTE=orc
RDIR=FastMultipole-024
STAGE=staging_024_cpu
...
ssh "$REMOTE" "cd $RDIR \
  ...
  && bash -lc 'module load julia \
    ...
    && FM024_GIT_COMMIT=$commit FM024_GIT_TREE=$tree FM024_GIT_WORKTREE=$worktree \
       ...
       MATRIX_OPERATOR_REFACTOR/scripts/cpu_024_run.sh'"
