...
# cluster and submit the 023f dense-CUDA validation + benchmark job.
#   bash MATRIX_OPERATOR_REFACTOR/scripts/cuda_023f_submit.sh
...
ssh "$REMOTE" "cd $RDIR \
  ...
  && cp -r staging_023f/src staging_023f/test . \
  && cp staging_023f/Project.toml . \
  && mkdir -p MATRIX_OPERATOR_REFACTOR \
  && cp -r staging_023f/scripts MATRIX_OPERATOR_REFACTOR/ \
  && bash -lc 'module load julia \
      && julia --project=\$HOME/fm023env -e \"using Pkg; Pkg.develop(path=\\\".\\\"); Pkg.instantiate()\" \
      && julia --project=test -e \"using Pkg; Pkg.develop(path=\\\".\\\"); Pkg.instantiate()\" \
      && sbatch MATRIX_OPERATOR_REFACTOR/scripts/cuda_023f_run.sh'"

echo "Submitted. Poll with:  ssh orc 'squeue -u \$USER'"
echo "Fetch results with:    bash MATRIX_OPERATOR_REFACTOR/scripts/cuda_023f_fetch.sh <jobid>"
