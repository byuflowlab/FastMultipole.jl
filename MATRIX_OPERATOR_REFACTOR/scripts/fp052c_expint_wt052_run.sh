...
# in the UNIFIED ~/projects/FLOWVPM.jl tree (installed 2026-09-05).
# wt052 WORKTREE variant (Ryan ruling 2026-09-05: one repo, frozen
# campaign-052 git worktrees under ~/wt052; silos retired after trial-2).
# Run data lives in the consolidated root via per-run symlinks.
...
FPDIR=$HOME/wt052/FLOWPanel.jl
VPMDIR=$HOME/wt052/FLOWVPM.jl
ENVDIR=$HOME/wt052/env-aarch64
DATA_ROOT=$HOME/projects/FLOWPanel.jl/data   # consolidated data root; run dirs symlinked in
...
mkdir -p "$DATA_ROOT/$run_name"
ln -sfn "$DATA_ROOT/$run_name" "data/$run_name"
...
env "${FM052_PRODUCTION_ENV[@]}" "${FM052_GPU_ENV[@]}" "${GUARD_ENV[@]}" \
  ...
  2>&1 | tee "$DATA_ROOT/${run_name}.log"
gpu_s_count=$(grep -c source_influence_s_gpu_gemv "$DATA_ROOT/${run_name}.log" || true)
cpu_s_count=$(grep -c source_influence_s_gemv "$DATA_ROOT/${run_name}.log" || true)
...
"$JULIA_BIN" --project="$ENVDIR" "$VPMDIR/scripts/fm052_compare.jl" report \
  "data/$run_name" "$DATA_ROOT/${run_name}.log"
"$JULIA_BIN" --project="$ENVDIR" "$VPMDIR/scripts/fm052_compare.jl" verify \
