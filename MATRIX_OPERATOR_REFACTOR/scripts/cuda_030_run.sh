...
#   retune     an explicit per-n candidate list (ell x radius schedule x float
#              type), used for the joint per-n retune campaign
...
if [ "$MODE" = "sweep" ]; then
...
elif [ "$MODE" = "retune" ]; then
    # Joint per-n retune (user direction 2026-08-04): the candidate list varies
    # depth, radius schedule, and float type together, so it cannot be generated
    # by a nested loop the way `sweep` is. FM030_RETUNE is a space-separated list
    # of
    #   <n>:<geometry>:<tf>:<fmt>
    # with the same geometry grammar as spotcheck (bare ell = the shipped
    # (6,5,...,5) schedule at that depth; sched<q2>-...-<qell> = explicit). The
    # list is generated by analyze_030_costmodel.jl and pasted in, so the
    # measured grid is exactly the pre-registered one.
    # The list is long (order 100 cases), so it is normally staged as a file and
    # named by FM030_RETUNE_FILE rather than squeezed through --export.
    if [ -n "${FM030_RETUNE_FILE:-}" ]; then
      [ -s "$FM030_RETUNE_FILE" ] || {
          echo "FM030_RETUNE_FILE=$FM030_RETUNE_FILE missing or empty" >&2; exit 1; }
      FM030_RETUNE="$(sed -e 's/#.*//' "$FM030_RETUNE_FILE" | tr '\n' ' ')"
      echo "=== retune case list from $FM030_RETUNE_FILE"
    fi
    : "${FM030_RETUNE:?retune mode requires FM030_RETUNE or FM030_RETUNE_FILE}"
    for spec in $FM030_RETUNE; do
      IFS=: read -r n geom tf fmt <<< "$spec"
      run_case retune "$geom" "$tf" "$fmt" "$n"
    done
    ...
    echo "unknown FM030_MODE=$MODE (expected sweep|retune|spotcheck)" >&2
    exit 1
fi
