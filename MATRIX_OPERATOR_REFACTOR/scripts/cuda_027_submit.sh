...
# cluster and submit the 027 hierarchical-CUDA validation + benchmark job.
#   bash MATRIX_OPERATOR_REFACTOR/scripts/cuda_027_submit.sh
...
ssh "$REMOTE" "cd $RDIR \
  ...
  && cp -r staging_027/src staging_027/test . \
  && cp staging_027/Project.toml . \
  && mkdir -p MATRIX_OPERATOR_REFACTOR \
  && cp -r staging_027/scripts MATRIX_OPERATOR_REFACTOR/ \
  && bash -lc 'module load julia \
      && julia --project=\$HOME/fm023env -e \"using Pkg; Pkg.develop(path=\\\".\\\"); Pkg.instantiate()\" \
      && julia --project=test -e \"using Pkg; Pkg.develop(path=\\\".\\\"); Pkg.instantiate()\" \
      && sbatch MATRIX_OPERATOR_REFACTOR/scripts/cuda_027_run.sh'"

echo "Submitted. Poll with:  ssh orc 'squeue -u \$USER'"
echo "Fetch results with:    bash MATRIX_OPERATOR_REFACTOR/scripts/cuda_027_fetch.sh <jobid>"
