...
# the BYU cluster and submit the 029 single-H200 baseline job.
#   bash MATRIX_OPERATOR_REFACTOR/scripts/cuda_029_submit.sh
...
ssh "$REMOTE" "cd $RDIR \
  ...
  && bash -lc 'module load julia/1.11.7-6bmogfl \
      ...
      && sbatch MATRIX_OPERATOR_REFACTOR/scripts/cuda_029_run.sh'"

echo "Submitted. Poll with:  ssh orc 'squeue -u \$USER'"
echo "Fetch results with:    bash MATRIX_OPERATOR_REFACTOR/scripts/cuda_029_fetch.sh <jobid>"
