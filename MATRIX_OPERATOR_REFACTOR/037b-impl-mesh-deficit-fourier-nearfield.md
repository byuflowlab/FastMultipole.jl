# 037b Nearfield Reduction II: Mesh/Fourier Deficit Evaluation (PME-Style)
...
Entry gate: `037a` Done and clear-context approved, AND its §6 verdict
recommends this row (with the quantitative basis recorded there), AND
explicit user go. If `037a` recommends against, this row is closed with a
...
Ewald exploits. Where `037a` evaluates the deficit pairwise within
...
against both the shipped baseline and `037a`'s best two-pass result — under
...
3. **Benchmark and report:** the `037a` case set (cube, wake, rotor wake;
   both scales, both precisions) with same-job anchors for (i) the shipped
   partitioned baseline and (ii) the `037a` winner, so the report can state
   exactly what each approach delivers and what this row adds over `037a`.
...
- `037a` (verdict + case set + decomposition instrument + anchors),
...
- Same as `037a`: 1e-3 conservative velocity gate on checksummed references
