...
function update_cuda_radix_state!(cache::RadixFMMCache{TF,LH}, systems::Tuple) where {TF,LH}
    ...
    # zero-M2L cache and re-runs the refresh from the top (the rebuild swaps
    # every cache field, including grid/context, so the partial refresh above
    # is discarded); the rebuilt cache's gate is vacuous, so the recursion
    # terminates.
