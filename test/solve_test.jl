...
@testset "Fast Gauss Seidel: matrices assembled vs probed (030 Phase 3)" begin

    n_bodies = 1000
    system = generate_gravitational(1234, n_bodies)

    # premise: the gravitational test system opts into block assembly
    @test FastMultipole.overrides_block_assembly(system)

    fgs_kwargs = (; expansion_order=4, multipole_acceptance=0.5, leaf_size=30)
    fgs_hook = FastMultipole.FastGaussSeidel((system,), (system,); fgs_kwargs...)
    fgs_probe = FastMultipole.FastGaussSeidel((system,), (system,);
        fgs_kwargs..., use_block_assembly=false)

    # premise: real nonself blocks exist at this leaf size
    @test length(fgs_hook.direct_list) > 0
    @test any(!iszero, fgs_probe.self_matrices.data)
    @test any(!iszero, fgs_probe.nonself_matrices.data)

    # route A (raw block through assemble_influence_block!, projected through
    # the same influence! call) vs the probe, rtol 1e-12 (not bitwise:
    # accumulation order differs)
    @test fgs_hook.self_matrices.sizes == fgs_probe.self_matrices.sizes
    @test fgs_hook.nonself_matrices.sizes == fgs_probe.nonself_matrices.sizes
    for k in eachindex(fgs_hook.self_matrices.sizes)
        mat_h, _ = FastMultipole.get_matrix_vector(fgs_hook.self_matrices, k)
        mat_p, _ = FastMultipole.get_matrix_vector(fgs_probe.self_matrices, k)
        @test isapprox(mat_h, mat_p; rtol=1e-12)
    end
    for k in eachindex(fgs_hook.nonself_matrices.sizes)
        mat_h, _ = FastMultipole.get_matrix_vector(fgs_hook.nonself_matrices, k)
        mat_p, _ = FastMultipole.get_matrix_vector(fgs_probe.nonself_matrices, k)
        @test isapprox(mat_h, mat_p; rtol=1e-12)
    end

    # full FGS solve through the hook-built matrices matches the probe-built
    # solver's residual history and solution
    original_strengths = [body.strength for body in system.bodies]
    function fgs_cold_solve!(system, solver)
        for (i, body) in enumerate(system.bodies)
            system.bodies[i] = typeof(body)(body.position, body.radius,
                                            original_strengths[i])
        end
        residuals = Float64[]
        FastMultipole.solve!(system, solver; scalar_potential=true,
            gradient=false, max_iterations=5, inner_iterations=2,
            tolerance=-1.0, final_update=false, verbose=false,
            callback=(_, residual) -> push!(residuals, residual))
        return residuals, [body.strength for body in system.bodies]
    end
    res_h, str_h = fgs_cold_solve!(system, fgs_hook)
    res_p, str_p = fgs_cold_solve!(system, fgs_probe)
    @test res_h ≈ res_p rtol=1e-10 atol=1e-12
    @test str_h ≈ str_p rtol=1e-10 atol=1e-12
