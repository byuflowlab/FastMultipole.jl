...
println("2. filament sources against the all-direct reference")
let n = 4000, ns = 400
    errs = Float64[]
    for (P, ell) in ((4, 1), (4, 3), (6, 3), (8, 3), (10, 3))
        Random.seed!(1)
        ...
        FM._launch_host_b2m!(st); FM.resident_extra_b2m!(st, binned)
        FM._launch_host_resident_operator_pipeline!(st)
        FM.resident_extra_near!(st, binned, FM.direct_kernel(ex))
        new = g(st.output) .- base
        e = maximum(abs.(new .- ref)) / maximum(abs, ref)
        push!(errs, e)
        @printf("   P=%2d ell=%d: %d cells, %d loose, relative %.3e\n", P, ell, nc, size(loose, 2), e)
    end
    check(errs[1] < 1e-12, @sprintf("every pair near is exact (%.1e)", errs[1]))
    check(issorted(errs[2:end]; rev = true) && errs[end] < errs[2] / 10,
