# Isolated M2M correctness check for the ext/FastMultipoleKAExt.jl KA building
# blocks (ka_gather_rotate_z!, ka_stacked_y_dense!, ka_rotate_z_scatter_accumulate!,
# ka_gather_rows!), wired together as ka_resident_stage_group_apply! -- the
# KA-ported form of the real production resident M2M path
# (_resident_stage_group_apply!, src/translate_batched.jl:2962, reached in
# production via _launch_resident_m2m!). Dispatch entry point is
# FastMultipole.ka_m2m_operator_batch!, which builds a single-group resident
# workspace on the fly (no tree required -- this checks the per-node math, not
# tree construction, per the ka-migration plan's step-2 scope) and compares
# against the CPU non-resident m2m_operator_batch! (MaterializedYRotationM2M),
# which src/translate_batched.jl documents as numerically agreeing with the
# resident dense-GEMM path (different factorization of the same M2M math).
...
println("Starting KA M2M correctness test...")
if !dev_functional()
...
for P in (4, 8)
    for LHbool in (true, false)
        for nbatch in (5, 10)
            ...
            r = TF(0.5) + TF(0.5) * rand(TF)            # resident M2M groups share one radius
            rs = fill(r, nbatch)
            ...
            FastMultipole.m2m_operator_batch!(op, targets_cpu, sources,
                                              phis, thetas, rs,
                                              cache, scratch, lh)

            # Device (Metal) path via the real ka_m2m_operator_batch! dispatch
            sources_metal = to_metal(sources)
            targets_metal = to_metal(FlatCoefficientBuffer(TF, cache.basis_info, nbatch))

            FastMultipole.ka_m2m_operator_batch!(op, targets_metal, sources_metal,
                                                 phis, thetas, rs,
                                                 cache, scratch, lh)
...
println("\n✓✓✓ All KA M2M correctness tests passed on $(DEV_NAME)! ✓✓✓")
