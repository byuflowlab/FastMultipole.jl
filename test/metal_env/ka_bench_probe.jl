...
for (ci, (P, ell, n, wc, trials)) in pairs(CASES)
    TF = Float32
    _t0 = time(); println("case $ci: P=$P ell=$ell n=$n"); flush(stdout)
    ...
    hcache = RadixFMMCache(sys_h; expansion_order=P, ell=ell, window_classes=wc,
        options=opts)
    fmm!(sys_h, hcache)
    println("  [stage] host RadixFMMCache build+fmm done: ", round(time()-_t0, digits=1), "s"); flush(stdout); 
    ...
    println("  [stage] device build done: ", round(time()-_t0, digits=1), "s"); flush(stdout); 
    ...
    println("  [stage] classic Cache build+fmm done: ", round(time()-_t0, digits=1), "s"); flush(stdout); 
    ...
    println("  [stage] entering metal timing"); flush(stdout)
    ...
    println("  [stage] metal timed done: ", round(time()-_t0, digits=1), "s"); flush(stdout); 
