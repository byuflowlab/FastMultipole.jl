...
const _T0 = time()
mark(x) = (println("[", round(time() - _T0, digits=1), "s] ", x); flush(stdout))
...
function make_field(kind::Symbol, seed, n, TF)
    ...
    elseif kind === :clustered
        nb = 8; c = rand(TF, 3, nb)
        hcat((c[:, 1 + (i % nb)] .+ TF(0.02) .* randn(TF, 3) for i in 0:(n-1))...)
    elseif kind === :planar
...
function arm1(label, hcache, dcache)
    ...
    if unstable
        ...
        hs = [Set(hg.body_index[hg.cell_ranges[1,c]:(hg.cell_ranges[1,c]+hg.cell_ranges[2,c]-1)])
              for c in 1:nc]
        dcr = A(dg.cell_ranges)[:, 1:nc]; dbi = A(dg.body_index)[1:nb]
...
function arm2(label, sys, hcache, dcache, ell, TF)
    ...
    dg = dcache.state.grid
    nb = hg.n_bodies; nc = hg.n_cells
    mark("arm2 building pos matrix");    pos = reduce(hcat, (b.position for b in sys.bodies))
...
for (kind, n, ell) in CASES
    ...
    before = nfail[]
    mark("build_pair enter");    hcache, dcache = build_pair(sys_h, sys_d, 4, ell, TF)
    mark("build_pair exit")
